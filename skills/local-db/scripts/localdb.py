...
def _split_sql(sql):
    """Split SQL on semicolons, respecting string literals, identifiers, and comments."""
    statements = []
    current = []
    in_single = False
    in_double = False
...
def cmd_execute(args):
    ...
    if _DESTRUCTIVE_PATTERNS.search(sql) and not args.allow_destructive:
        print(
    ...
    try:
        ...
        # Always commit if any DML was executed, regardless of last statement type
        if total_affected > 0:
            conn.commit()

        # Detect result-returning queries via cursor.description (handles
        # SELECT, PRAGMA, WITH ... SELECT, VALUES, etc.)
...
def cmd_migrate(args):
    ...
    if _DESTRUCTIVE_PATTERNS.search(args.sql):
        print(
